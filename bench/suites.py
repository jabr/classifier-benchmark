...
from bench.cases import TASKS_V1, TASKS_V2, load_suite, question_payload
...
# Real-data sample suites generated by sources/generate.py (see sources/README.md):
# one committed TOML per sample in sources/samples/, named <source>-<seed-hex>,
# each carrying full provenance in its header comments. They are unlocked and
# excluded from 'all'; select them with --sample (bench/run.py).
SAMPLES_DIR = Path(__file__).resolve().parents[1] / "sources" / "samples"
SAMPLE_SUITES: dict[str, list] = {}
if SAMPLES_DIR.is_dir():
  for _path in sorted(SAMPLES_DIR.glob("*.toml")):
    if _path.stem in SUITES:
      raise ValueError(f"sample suite name {_path.stem!r} collides with a core suite")
    SAMPLE_SUITES[_path.stem] = load_suite(_path)
SUITES.update(SAMPLE_SUITES)
...
def resolve_suites(value: str) -> list[str]:
  """Parse a comma-separated core-suite list; 'all' expands to the core suites
  (v1, v2) only. Generated sample suites are a separate selection axis — see
  resolve_samples (used by --sample in bench.run)."""
  wanted = [s.strip() for s in value.split(",") if s.strip()] or ["all"]
  names: list[str] = []
  seen: set[str] = set()
  for name in wanted:
    if name == "all":
      candidates = [s for s in SUITES if s not in SAMPLE_SUITES]
    else:
      if name in SAMPLE_SUITES:
        raise ValueError(f"{name} is a generated sample suite; select it with --sample")
      if name not in SUITES:
        raise ValueError(f"Unknown suite {name!r}. Available: {sorted(set(SUITES) - set(SAMPLE_SUITES))} (samples: use --sample)")
      candidates = [name]
    for c in candidates:
      if c not in seen:
        seen.add(c)
...
def resolve_samples(value: str) -> list[str]:
  """Parse a comma-separated sample-suite list; 'all' expands to every
  discovered sample suite, erroring when none are discovered."""
  discovered = sorted(SAMPLE_SUITES)
  wanted = [s.strip() for s in value.split(",") if s.strip()] or ["all"]
  if not discovered:
    raise ValueError("No generated samples discovered in sources/samples (generate one with: just gen <source>)")
  names: list[str] = []
  seen: set[str] = set()
  for name in wanted:
    candidates = discovered if name == "all" else [name]
    if name != "all" and name not in SAMPLE_SUITES:
      raise ValueError(f"Unknown sample {name!r}. Discovered samples: {discovered}")
    for c in candidates:
      if c not in seen:
        seen.add(c)
        names.append(c)
